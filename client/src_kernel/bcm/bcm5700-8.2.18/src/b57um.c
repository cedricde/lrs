				       ...
				       const struct pci_device_id *ent)
{
	...
	if ((pDevice->PhyId & PHY_ID_MASK) == PHY_BCM5400_PHY_ID)
        ...
        else if ((pDevice->PhyId & PHY_ID_MASK) == PHY_BCM5715_PHY_ID)
                printk("Broadcom BCM5715 Integrated Copper ");
	...
	if ((pDevice->TaskToOffload & LM_TASK_OFFLOAD_TCP_SEGMENTATION) &&
		...
		if( !(T3_ASIC_5714_FAMILY(pDevice->ChipRevId) && (dev->mtu > 1500)) ){
			dev->features |= NETIF_F_TSO;
		}else{
			printk(KERN_ALERT "Jumbo Frames and TSO cannot simultaneously be enabled. Jumbo Frames enabled. TSO disabled.\n");
...
STATIC int bcm5700_change_mtu(struct net_device *dev, int new_mtu)
{
	...
	if(T3_ASIC_5714_FAMILY(pDevice->ChipRevId) &&
	  (dev->mtu > 1514) ){
		if(dev->features & NETIF_F_TSO){
			dev->features &= ~NETIF_F_TSO;
			printk(KERN_ALERT "TSO previously enabled. Jumbo Frames and TSO cannot simultaneously be enabled. Jumbo Frames enabled. TSO disabled.\n");
		}
	}
#endif
