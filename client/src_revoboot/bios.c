...
#ifdef DEBUG
      printf("Int 13 check returned : %d\n",version);
#endif
      ...
      if (version)
	{
...
#ifdef DEBUG
	  printf("Get disk info (int13/48) : %d %d\n",drp.size,err);
#endif
	 ...
	 so that we can change the structure easily.  */
#ifdef DEBUG
      grub_printf("get_diskinfo_standard called\n");
#endif
      err = get_diskinfo_standard (drive,
				   ...
				   &geometry->sectors);
#ifdef DEBUG
      grub_printf("returned %d %dx%dx%d\n", err, geometry->cylinders,geometry->heads,geometry->sectors);
#endif
      if (err)
	return err;
