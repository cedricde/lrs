...
static int _search_by_entry_key (reiserfs_filsys_t fs, struct key * key, 
				 struct path * path)
{
    struct buffer_head * bh;
    int item_pos;
...
/* add new name into a directory. If it exists in a directory - do
   nothing */
void reiserfs_add_entry (reiserfs_filsys_t fs, struct key * dir, char * name,
			 struct key * key, int fsck_need, int lost_found)
{
    ...
    /* fsck may need to mark entry Lost+Found */
    if (lost_found)
	mark_de_lost_found ((struct reiserfs_de_head *)entry);
