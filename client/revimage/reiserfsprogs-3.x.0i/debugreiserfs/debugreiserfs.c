...
static void do_corrupt_one_block (reiserfs_filsys_t fs, int block)
{
	...
	case 'I':
	    /* break unformatted node pointer */
...
static void print_journal (struct super_block * s)
{
    ...
    reiserfs_warning (stderr, "Start scanning from %d\n", start_journal);
	...
	if (who_is_this (d_bh->b_data, d_bh->b_size) == THE_JDESC) {
		...
		reiserfs_warning (stderr, "-- wrapped?");
		wrapped = 1;
		break;
	    }
		...
		reiserfs_warning (stderr, "commit block %d (trans_id %ld, j_len %ld) does not match\n", commit_block,
	    ...
	    valid_transactions ++;
	    reiserfs_warning (stderr, "(commit block %d) - logged blocks (", commit_block);
#if 0
		...
		if (not_journalable (s, block))
		    reiserfs_warning (stderr, " xxxx");
		else {
		    reiserfs_warning (stderr, " %ld", desc->j_realblock[j]);
		    if (block_of_bitmap (s, desc->j_realblock[j]))
			reiserfs_warning (stderr, "(bmp)");
		}
		if (j && (j + 1) % 10 == 0)
	...
	commit = bh_commit (c_bh);
	if (does_desc_match_commit (desc, commit)) {
	    reiserfs_warning (stderr, "No! commit block %d (trans_id %ld, j_len %ld) does not match\n",
			       first_desc_block - 1, commit->j_trans_id, commit->j_len);
	} else {
	    reiserfs_warning (stderr, "Yes! (commit block %d) - logged blocks (\n", first_desc_block - 1);
#if 0
	    for (j = 0; j < desc->j_len; j ++) {
		unsigned long block;
		...
		if (not_journalable (s, block))
		    ...
		    reiserfs_warning (" %ld", desc->j_realblock[j]);
		    if (block_of_bitmap (s, desc->j_realblock[j]))
			reiserfs_warning (stderr, "(bmp)");
		}
	    }
    ...
    reiserfs_warning (stderr, "%d valid transactions found\n", valid_transactions);
...
static void look_for_key (struct buffer_head * bh, struct key * key)
{
	...
	if (ih->ih_key.k_dir_id == key->k_dir_id &&
	    ih->ih_key.k_objectid == key->k_objectid) {
	    fprintf (log, "block %lu has item of file %u %u (item %d)\n",
		     bh->b_blocknr, key->k_dir_id, key->k_objectid, i);fflush (log);
	    return;
	}
    }
    return;
...
static void do_scan (reiserfs_filsys_t fs)
{
	...
	brelse (bh);
	print_how_far (&done, SB_BLOCK_COUNT (fs), 1, 0);
    }
}
...
static void do_test (reiserfs_filsys_t fs)
{
	...
	reiserfs_add_entry (fs, &root_dir_key, "lost+found", &root_dir_key, 0, 0);
}
