...
#define sb_size(s) (SB_VERSION(s) == REISERFS_VERSION_2) ? SB_SIZE : SB_SIZE_V1
...
#define DEH_Lost_Found 3 /* fsck sets this bit for names linked during
			    "lost+found" pass. Semantic pass then
			    clear it */
...
#define de_lost_found(deh)          test_bit (DEH_Lost_Found, &((deh)->deh_state))
...
/*char * st_mode2string (short unsigned int mode, char *);*/
char * hash_name (int code);
#endif
				   ...
				   struct reiserfs_de_head * deh, int i)
{
    ...
    char * name = name_in_entry (deh, i);

#ifdef CONFIG_REISERFS_CHECK
    if (de_with_sd (deh) || !name[0] || !len)
	reiserfs_panic (0, "name_length: stat data in entry or wrong entry");
#endif
