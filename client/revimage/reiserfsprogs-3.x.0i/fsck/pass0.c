				  ...
				  int item_num)
{
	...
	if (i == 0 && is_dot (name, name_len)) {
		...
		if (code2func (hash_code) != 0) {
			...
			fsck_log ("pass0: block %lu, item %H: entry %d. found a name \"%.*s\" "
				  "matching to deh_offset %x. FIXME: should set deh_location "
	...
	/* to delete entries of a directory item we need to have
	   reasonable deh_location-s */
...
/* block which are to be read are marked in fsck_disk_bitmap (fs) */
static int how_many_to_scan (reiserfs_filsys_t fs, unsigned long first, int count)
{
    int i;
    int used;
    ...
    used = 0;
    for (i = 0; i < count; i ++) {
	if ((SB_BLOCK_COUNT (fs) > (first + i)) && is_to_be_read (fs, first + i))
	    used ++;
    }
    return used;
}
...
static void go_through (reiserfs_filsys_t fs)
{
    ...
    fsck_progress ("Pass 0 (%lu (of %lu) blocks will be read):\n",
	...
	if (to_scan) {
		...
		for (j = 0; j < nr_to_read; j ++) {
			...
			reiserfs_panic ("not dats block found");
...
static void choose_hash_function (reiserfs_filsys_t fs)
{
	...
	/*
	if (!fsck_user_confirmed (fs, "Overwite?(Yes)", "Yes", 1)) {
