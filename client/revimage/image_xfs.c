...
__u64 convertAgbToDaddr(__u32 agno, __u32 Agblockno)
{
  ...
  FSBlock = (((__u64)agno)*((__u64)ntohl(sb.sb_agblocks)))+((__u64)Agblockno);
...
void addtohist(PARAMS *p, __u32 agno, __u32 Agblockno, __u64 Len)
{
  ...
  Base = (((__u64)agno) * ((__u64)ntohl(sb.sb_agblocks))) + ((__u64)Agblockno);
  // debug("%lld , %lld\n", Base, Len);
  for (i=0; i < Len; i++) {
...
void scanfuncBno(int fd, PARAMS *p, char *ablock, __u32 level, struct xfs_agf *agf)
{
  ...
  if (level == 0)
	...
	addtohist(p, ntohl(agf->agf_seqno), ntohl(rp[i].ar_startblock), ntohl(rp[i].ar_blockcount));
      }
      return;
    }
    
  mxr  = (int)((4096 - (uint)sizeof(xfs_alloc_block)) / ((uint)sizeof(xfs_alloc_key) + (uint)sizeof(xfs_alloc_ptr)));
  pp = (xfs_alloc_ptr *)((char *)(ablock) + sizeof(xfs_alloc_block) + (mxr * sizeof(xfs_alloc_key)));      
  for (i = 0; i < ntohs(block->bb_numrecs); i++)
    {
...
void readsb(PARAMS *p, char *dev)
{
   ...
   fd = open (dev, O_RDONLY);
   if (fd == -1) exit(1);  
   if (read(fd, &sb, sizeof(xfs_superblock)) != sizeof(xfs_superblock)) exit(1);
   ...
   for (i = 0; i < agcount ; i++)
       ...
       if ((ntohl(agf.agf_magicnum)) != XFS_AGF_MAGIC)
	 {
       ...
       scanSbtree(fd, p, &agf, ntohl(agf.agf_roots[XFS_BTNUM_BNO]), ntohl(agf.agf_levels[XFS_BTNUM_BNO]));
