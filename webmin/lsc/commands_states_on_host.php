...
if ($_GET["id_command_on_host"]!="") {
	$current_tab="commands_states_on_host";
...
require_once(dirname(__FILE__) . "/include/commands.inc.php");
 ...
 */
$template = new LSC_Tmpl(array("commands_states_on_host_page" => "commands_states_on_host_page.tpl" ));
 ...
 */
 if ($number_command_by_page==10) {
	 $template->set_block("commands_states_on_host_page", "NUMBER_BY_PAGE_10_SELECTED", "page_10_selected");
 ...
 if ($number_command_by_page==20) {
	 $template->set_block("commands_states_on_host_page", "NUMBER_BY_PAGE_20_SELECTED", "page_20_selected");
...
if ($number_command_by_page==50) {
	 $template->set_block("commands_states_on_host_page", "NUMBER_BY_PAGE_50_SELECTED", "page_50_selected");
 ...
 if ($number_command_by_page==100) {
	 $template->set_block("commands_states_on_host_page", "NUMBER_BY_PAGE_100_SELECTED", "page_100_selected");
...
"
SELECT
	DATE_FORMAT(A.date_created, '%%d-%%m-%%Y<br />%%H:%%i:%%s'),
	DATE_FORMAT(A.start_date, '%%d-%%m-%%Y<br />%%H:%%i:%%s'),
	DATE_FORMAT(A.end_date, '%%d-%%m-%%Y<br />%%H:%%i:%%s'),
	A.id_command,
	B.id_command_on_host,
	A.title,
	A.date_created,
	A.start_date,
	A.end_date,
	...
	while($database->next_record()) {
		array_push($commands_array, 
			array(
				...
				"date_created_formated" => $database->f(0),
				"start_date" => $database->f("start_date"),
				"start_date_formated" => $database->f(1),
				"end_date" => $database->f("end_date"),
				"end_date_formated" => $database->f(2),
