...
$template->set_var("SCRIPT_NAME", "all_commands.cgi");
...
/**
 * Display target_filter list
 */

$query_target_filter_list = sprintf(
"
SELECT
	DISTINCT(target)
FROM
...
$database->query($query_target_filter_list);
...
if ($database->num_rows() > 0) {
	while($database->next_record()) {
		$template->set_var("TARGET_FILTER", $database->f("target"));
		if ($database->f("target")==$target_filter) {
 ...
 */
 if ($number_command_by_page==10) {
	 $template->set_block("all_commands_states_page", "NUMBER_BY_PAGE_10_SELECTED", "page_10_selected");
 ...
 if ($number_command_by_page==20) {
	 $template->set_block("all_commands_states_page", "NUMBER_BY_PAGE_20_SELECTED", "page_20_selected");
...
if ($number_command_by_page==50) {
	 $template->set_block("all_commands_states_page", "NUMBER_BY_PAGE_50_SELECTED", "page_50_selected");
 ...
 if ($number_command_by_page==100) {
	 $template->set_block("all_commands_states_page", "NUMBER_BY_PAGE_100_SELECTED", "page_100_selected");
