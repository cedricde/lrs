...
// values from host file
  ...
  if (!isset($shares) && !($xfer_changed)) {
    ...
    if ($xfermethod == "rsyncd")
      $reg = "/[\$]Conf\{RsyncShareName\}\s*=\s*\[(.*)\];/";
    else 
    if ($xfermethod == "tarssh")
      $reg = "/[\$]Conf\{TarShareName\}\s*=\s*\[(.*)\];/";
    else
      $reg = "/[\$]Conf\{".$str."ShareName\}\s*=\s*\[(.*)\];/";
    preg_match($reg, $filestr, $m);
...
function generateSshKey() {
  if(!file_exists('/var/lib/backuppc/.ssh'))
    mkdir("/var/lib/backuppc/.ssh", 0644);
  if (!file_exists('/var/lib/backuppc/.ssh/id_rsa.pub')) {
    if (!file_exists('~/.ssh/id_rsa.pub')) 
      exec("ssh-keygen -t rsa -N '' -f ~/.ssh/id_rsa");
    
    copy("/root/.ssh/id_rsa", "/var/lib/backuppc/.ssh/id_rsa");
    copy("/root/.ssh/id_rsa.pub", "/var/lib/backuppc/.ssh/id_rsa.pub");
  }
  if (!file_exists('/var/lib/backuppc/.ssh/BackupPC_id_rsa.pub')) {
    copy("/root/.ssh/id_rsa.pub", "/var/lib/backuppc/.ssh/BackupPC_id_rsa.pub");
  }
}
