...
sub init() {
        ...
        $ENV{"FOREIGN_MODULE_NAME"}="lbs-inventory";
#        main::init_config();
...
my $hashref=shift;
        ...
        if (defined($hashref->{'mac'})) {
                ...
                my $LRSfile=$lbs_inventory::config{'chemin_basedir'} . "/log/" . main::mac_remove_columns($mac) . ".ini";
#                my $LRSfile=$lbs::config{'chemin_LBS'} . "/" . main::mac_remove_columns($mac) . ".ini";
                ...
                my $OCSfile=$lbs_inventory::config{'chemin_CSV'}."/Network/".main::mac_remove_columns($mac);
                ...
                my $link = "<div title=\"$timestamp\" 
                              ...
                              <a
                                        onmouseover=\"$name.src='$imgover';\"
                                        onmouseout=\"$name.src='$imgout';\"
                                        href=\"/$module_name/groups.cgi?$params\">
                                <img
                                        name=\"$name\"
                                        border='0'
                                        src=\"$imgout\"
