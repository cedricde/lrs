...
$temp = $GLOBALS["config"]; # <- default lrs-inventory config
if ($temp == -1) 
{ # if doesn't exist, copy the default in /etc/webmin/lrs-inventory/config (default)
  $assocTable = lib_read_file("./config");
  print "<h1> error configuration file not found, using default</h1> <br> <font size = '5' weight='bold'>Copying config into default directory (/etc/webmin/lrs-inventory) <br> Please reload the page <br> <br> </font> If you see this page a second time, please copy the config file into your lrs-inventory configuration directory <br>";
`cp ./config /etc/webmin/lrs-inventory/`;
}
