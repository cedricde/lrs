...
$Qtpl::VERSION = '0.5-lbx';
#$Qtpl::file_delim = "\{FILE\s*\"(.+?)\"\s*\}";
$Qtpl::file_delim = '\{FILE\s*\"(.+?)\"\s*\}';
$Qtpl::block_start_delim = '<!-- ';
$Qtpl::block_end_delim = '-->';
#$Qtpl::block_start_word = 'BEGIN:';
#$Qtpl::block_end_word = 'END:';
$Qtpl::block_start_word = 'BEGIN';
$Qtpl::block_end_word = 'END';
$Qtpl::var_begin = '{';
$Qtpl::var_end = '}';  
...
sub block_start_delim {
	...
	$ret = $Qtpl::block_start_delim;
	$Qtpl::block_start_delim = $v if (length $v);
	return $ret;
}
...
sub block_end_delim {
	...
	my $ret;
	$v = shift if ((ref $v) eq 'Template::Qtpl');
	$ret = $Qtpl::block_end_delim;
	$Qtpl::block_end_delim = $v if (length $v);
	return $ret;
}
...
sub block_start_word {
	...
	my $ret;
	$v = shift if ((ref $v) eq 'Template::Qtpl');
	$ret = $Qtpl::block_start_word;
	$Qtpl::block_start_word = $v if (length $v);
	return $ret;
}
...
sub block_end_word {
	...
	my $ret;
	$v = shift if ((ref $v) eq 'Template::Qtpl');
	$ret = $Qtpl::block_end_word;
	$Qtpl::block_end_word = $v if (length $v);
	return $ret;
}
...
sub _if_subst {
	...
	my @parts = split(/$Qtpl::var_begin[E]LSE$Qtpl::var_end/, $body);
	if (length $if_val) {
		return $parts[0];
	} else {
		return $parts[1];
...
sub parse {
	...
	$self->set_error("parse: blockname [$bname] does not exist")
        ...
        while ($self->{blocks}->{$bname} =~ 
        		/$Qtpl::var_begin([\~\w\.\:]+)$Qtpl::var_end/g) {                                                                   
		my $v = $1;
		next if ($v =~ /^IF\s+/);
		...
		if ($v =~ /^_BLOCK_\.(.+)$/) {
			...
			$copy =~ s/$Qtpl::var_begin$v$Qtpl::var_end/$var/eg;
		} else {
...
# parse if tags
	$copy =~ s/$Qtpl::var_begin[I]F\s+(.+?)\s*$Qtpl::var_end(.*?)$Qtpl::var_begin[E]NDIF$Qtpl::var_end/$self->_if_subst($1, $2)/egs;
...
sub maketree {
	my $self = shift;
	my ($con, $block) = @_;
	my @con2=split($Qtpl::block_start_delim,$con);
	my $level=0;
	my @block_names = ('');
	my %blocks=();
	my $parent_name;
	...
	my %added_to_parent = ();
	foreach $v (@con2){
		my $patt="($Qtpl::block_start_word|$Qtpl::block_end_word)\\s*([\\w\\.\\:]+)\\s*$Qtpl::block_end_delim(.*)";
		if ($v =~ /$patt/is) {
			# $res[1] = BEGIN or END
			# $res[2] = block name
			# $res[3] = kinda content
			...
			$res[2] = $2;
			$res[3] = $3;
			if ($res[1] eq $Qtpl::block_start_word) {
				...
				if (!$added_to_parent{$cur_block_name_new}) {
					$blocks{$parent_name}.=$Qtpl::var_begin."_BLOCK_.$cur_block_name_new".$Qtpl::var_end;
			...
			} elsif ($res[1] eq $Qtpl::block_end_word) {
				...
				$blocks{$parent_name}.=$res[3];	
					#/* add rest of block to parent block */
                        ...
                        $parent_name=join(".",@block_names);                 
                        $parent_name =~ s/^\.//; #hack                       
                        $blocks{$parent_name}.=$Qtpl::block_start_delim . $v;
		}                                                              		}
	}
	return \%blocks;	
}
