...
#
# Copyright (C) 2003-2007 Free & ALter Soft, Linbox, Ludovic Drolez
#
...
function lib_init_config(){
	...
	// change the lang according to the browser settings
	if ($gconfig{'acceptlang'}) {
		$langs = (file("$webmin_path/lang_list.txt"));
		foreach (preg_split("/,/", $_SERVER{'HTTP_ACCEPT_LANGUAGE'}) as $testlang) {
			if (preg_grep("/lang=$testlang/", $langs)) {
				$current_lang = $testlang;
				break;
			}
		}
	}
...
function lib_load_language(){
	...
	global $webmin_path, $gconfig, $module_name, $remote_user, $module_directory, $current_lang;
	
	// should we backport webmin's @lang_order_list to phpwebmin ??
	$lang = $current_lang;	
	$text = lib_read_file($webmin_path."/lang/".$lang);

	//secondly with module text codes: /path/to/webmin/current_module/lang/current_language
	if ($module_name) {
